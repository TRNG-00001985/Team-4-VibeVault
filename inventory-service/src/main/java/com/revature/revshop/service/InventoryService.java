...
//    }
    public List<Boolean> areItemsInStock(List<OrderLineItemsDto> orderLineItems) {
        ...
        for (OrderLineItemsDto olidto : orderLineItems) {
            // Fetch inventory using Optional to avoid NullPointerException
            Optional<Inventory> inventoryOpt = inventoryRepository.findInventoryBySkuCode(olidto.getSkuCode());

            // Check inventory presence and status
            Boolean status = inventoryOpt
                .map(inventory -> inventory.getStatus().equals(Status.AVAILABLE))
                .orElse(false);  // Return false if inventory is not present
    ...
    public InventoryResponse getInventoryByUserId(String userId) {
        Inventory inventory = inventoryRepository.findByUserId(userId)
                .orElseThrow(() -> new RuntimeException("Inventory item not found for user ID: " + userId));
        return mapToInventoryResponse(inventory);
    }
