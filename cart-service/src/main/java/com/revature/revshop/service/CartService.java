...
public class CartService {
    ...
    // Fetch product details using WebClient
    public ProductDto getProductDetails(Long productId) {
        System.out.println("Fetching product details for product ID: " + productId);
        return webClient.get()
            .uri("/{id}", productId)  // Appends to the base URL set in WebClientConfig
            .retrieve()
    ...
    // Create a CartItem object from ProductDto and quantity
    private CartItem createCartItem(ProductDto product, int quantity) {
        return CartItem.builder()
                .productId(product.getProductId())  // Map productId from ProductDto
                .name(product.getProductName())     // Use productName instead of name
                .price(product.getPrice())          // Map price from ProductDto (BigDecimal)
                .quantity(quantity)                 // Use the passed quantity
                .skuCode(product.getSkuCode())      // Map skuCode from ProductDto
                .build();
    }
    ...
    private void updateOrAddCartItem(Cart cart, CartItem item) {
        ...
        Optional<CartItem> existingItem = cart.getCartItems().stream()
                .filter(cartItem -> cartItem.getProductId() == item.getProductId()) // Use `==` for int comparison
                .findFirst();
    ...
    public Cart removeItemFromCart(String userId, Long productId) {
        ...
        cart.getCartItems().removeIf(item -> Long.valueOf(item.getProductId()).equals(productId));
