...
//		this.kafkaTemplate = kafkaTemplate;
	...
	public String placeOrder(OrderRequest orderRequest) {
	    ...
	    for (Boolean inStock : itemsInStock) {
	        if (!inStock) {
	            return "Some items are not in stock";
	        }
	    }
	 ...
	 // Publish the order event to Kafka
	    // kafkaTemplate.send("notificationTopic", new OrderEvent(this, order.getOrderNumber(), userId, order.getStatus()));
	    //System.out.println("Notification sent");

	  //  return "Order created successfully with order number: " + order.getOrderNumber();
	}
